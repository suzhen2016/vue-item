...
export default {
  ...
  computed: {
    // Getting Vuex State from store/modules/subject 检测数据更新到本组件来使用；
    ...mapState({
      ...
      movieTags (state) {
        console.log(' 影评详情 ', state)
        return state.movie.movieTags
      }
    }),
  ...
  created () { // 初始化方法执行
    // Getting route params  根据电影id来查询改影评的详情请求;
    const id = this.$route.params.id
    const classify = this.$route.params.classify
    console.log('电影id的请求参数', id, classify)
    // Dispatching getSingleSubject   请求actions内的type方法
    this.$store.dispatch({
